...
public class SpringBootDemo1ApplicationTests {
	...
	public void test() {
		...
		System.out.println("-=========***********=-===");
		Assertions.assertEquals(5, userService.getAllUsers().intValue());
		System.out.println("-========***********-=-===");
...
//		userSerivce.deleteAllUsers();
		...
		// 查数据库，应该有5个用户
		Assertions.assertEquals(3, userService.getAllUsers().intValue());
...
//		workerRepository.deleteAll();
		...
		Assertions.assertEquals(60, workerRepository.findByName("FFF").getAge().longValue());

        // 测试findUser, 查询姓名为FFF的Worker
		Assertions.assertEquals(60, workerRepository.findWorker("FFF").getAge().longValue());

        // 测试findByNameAndAge, 查询姓名为FFF并且年龄为60的User
		Assertions.assertEquals("FFF", workerRepository.findByNameAndAge("FFF", 60).getName());

        // 测试删除姓名为AAA的Worker
        workerRepository.delete(workerRepository.findByName("AAA"));

        // 测试findAll, 查询所有记录, 验证上面的删除是否成功
		Assertions.assertEquals(9, workerRepository.findAll().size());
    ...
    public void testRedis(){
		...
		Assertions.assertEquals(20, ((User2)redisTemplate.opsForValue().get("超人")).getAge().longValue());
		Assertions.assertEquals(30, ((User2)redisTemplate.opsForValue().get("蝙蝠侠")).getAge().longValue());
		Assertions.assertEquals(40, ((User2)redisTemplate.opsForValue().get("蜘蛛侠")).getAge().longValue());
